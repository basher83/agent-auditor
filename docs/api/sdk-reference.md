...
```bash
# Audit a skill (from package root)
...
src/
├── skill_auditor/
│   ├── cli.py                    # Main SDK application (entry point)
...
└── tests/
    ...
    └── test_b4_patterns.py       # Tests for B4 pattern detection
```
...
```bash
# From package root
python -m pytest src/tests/test_metrics_extractor.py  # 21 extraction tests
python -m pytest src/tests/test_skill_auditor.py      # 12 application logic tests
python -m pytest src/tests/test_determinism.py        # 2 determinism integration tests
python -m pytest src/tests/test_b4_patterns.py       # B4 pattern detection tests
```
